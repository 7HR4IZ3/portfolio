...
class DictDatabase(DB):
	...
	def __init__(self, path="database.json", encrypt=False):
		super(DictDatabase, self).__init__(_tables = Dict(parent=self), _schema = Dict(parent=self))
	...
	def table(self, name: str, _columns: Dict) -> Table:
		'''
...
db = DictDatabase("database.dictdb")
